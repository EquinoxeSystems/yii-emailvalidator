...
/***************************************************************************
 ...
 Configuration
  ...
  If you are working in win32 environment and don't set $this->_NAMESERVERS
...
class rrcompat
{
   ...
   {
   	if (!isset($this->_NAMESERVERS) || !is_array($this->_NAMESERVERS)) {
   		...
   		if (strncmp(PHP_OS, "WIN", 3) == 0) {
   			...
   			for ($i=0; $i<$cnt; ++$i) {
   				if (strpos($res[$i], 'DNS Servers') !== false) {
   					$this->_NAMESERVERS[] = trim(substr($res[$i], strpos($res[$i], ': ') + 2));
   					break;
   				}
   			}
   	...
   	if (count($this->_NAMESERVERS) > 0) {
         $this->__PHPRESOLVER_RS = new DNSResolver($this->_NAMESERVERS[0]);
   	}
   	else {
         $this->__PHPRESOLVER_RS = false;
   	}
   }

	public function _checkdnsrr($hostname, $type = 'MX')
	{
		...
		if ($this->__PHPRESOLVER_RS === false) return false;
		...
		$rt = $typemap[$type];
		$answer = &$this->__PHPRESOLVER_RS->sendQuery(new DNSQuery(new DNSRecord($dnsname, $rt)));
	...
	public function GetMXRR($hostname, &$mxhosts, &$weight)
	{
		...
		$dnsname = &DNSName::newFromString($hostname);
		$answer = &$this->__PHPRESOLVER_RS->sendQuery(new DNSQuery(new DNSRecord($dnsname, DNS_RECORDTYPE_MX)));
