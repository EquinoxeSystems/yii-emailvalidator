...
class	DNSRecord
{
   ...
   public function DNSRecord($name, $type, $dclass = DNS_CLASS_INTERNET, $ttl = 0, $specific_fields = false )
   {
      $this->name = $name;
      $this->type = $type;
      $this->dclass = $dclass;
	...
	public function &getTypeSpecificField($name)
	{
...
class	DNSResolver
{
   ...
   public function DNSResolver($nameserver, $port=53, $timeout=1000000)
   {
      $this->port = $port;
	...
	public function sendQuery($dnsquery, $useTCP=false)
   {
      $answer = false;
      ...
      if ($out_buf) {
         if ($useTCP == false && $out_buf_len <= DNS_UDP_PACKET_MAX_LENGTH) {
            ...
            /* connection by TCP */
				...
				socket_set_blocking($sock, true);
				if (fwrite($sock, pack('n', $out_buf_len)) == 2 && fwrite($sock, $out_buf) == $out_buf_len) {
...
class	DNSQuery
{
	...
	public function DNSQuery(&$dnsrecord, $flags=DNS_HEADERSPEC_RECURSION_DESIRED)
	{
...
class	DNSMessageParser
{
	...
	public function DNSMessageParser($stream, $limit)
	{
		$this->stream = $stream;
		$this->nbytes_read = 0;
		$this->octets = '';
	...
	public function readStreamDirectly($nbytes)
	{
      if (($this->limit -= $nbytes) < 0 || $nbytes <= 0) {
			return false;
		}
	...
	public function readBufferedStream($nbytes, $offset)
	{
   ...
   public function getHeaderInfo()
   {
	...
	public function getQueryRecords($nrecs)
	{
		$recs = array();
		while (--$nrecs >= 0) {
         if (($labels = $this->getLabels()) === false) {
			...
			$recs[] = new DNSRecord(new DNSName($labels), $info['type'], $info['dclass']);
		}
		return $recs;
	}

	public function getResourceRecords($nrecs)
	{
      $recs = array();
		while (--$nrecs >= 0) {
         if (($labels = $this->getLabels()) === false) {
	...
	public function getLabels($max_length=255, $offset=-1)
	{
      if ($offset < 0) {
         $offset = $this->nbytes_read;
      }
		$labels = array();
		for (;;) {
			if (--$max_length < 0) {
...
class	DNSAnswer
{
   ...
   public function DNSAnswer(&$stream, $limit)
   {
...
class	DNSName
{
   ...
   public function DNSName($labels)
   {
		$this->labels = &$labels;
	}

	public function isRealDomainName()
	{
   ...
   public function newFromString($domain_name)
   {
      if (strpos($domain_name, ':') !== false && !preg_match('/[^0-9a-fA-f:.]/', $domain_name)) {
			...
			if ($nlabels == 5 && !preg_match('/[^0-9.]/', $domain_name)) {
